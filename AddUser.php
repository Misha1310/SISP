...
$errPass = 'Password must be at least 8 characters and must contain at least one lower case letter, one upper case letter and one digit';
  ...
  $new =$_POST["id_password"];  
    ...
    $role=$_POST["role"];
    if( !$errPass ) {  
    $sql_query="SELECT * FROM logindb WHERE uid='$uid'";
    $result=mysqli_query($conn,$sql_query);
    if(mysqli_num_rows($result)==0)
    {
    if($new==$conf)
    {
      $sql="INSERT INTO logindb VALUES ('$uid','$new','$role') ";
     ...
     echo '<script type="text/javascript">
          ...
          window.onload = function () { alert("PASSWORD AND CONFIRM PASSWORD FIELD DOES NOT MATCH!!"); }
      ...
      echo '<script type="text/javascript">
           ...
           window.onload = function () { alert("USER ID ALREADY EXIST!!"); }
...
<body>
  ...
  <li class="dropdown" style="float:right">
    ...
    <div class="wrapper">
        ...
        <div class="btn">
        <button type="submit" name="adduser" class="login-btn"><i class="fa-solid fa-user-plus awesome"></i>Add</button>
        </div>
    </div>
    </form>
</div>
